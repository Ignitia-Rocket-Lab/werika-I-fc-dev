...
/* config defines */
#define NRF_RFCHANNEL_ADDR_LENS_IN_BYTE 5 // Valid from 3 - 5 Bytes width
...
#define nRF_CONFIG                                                             \
  0x00 // Configurate the status of transceiver, mode of CRC and the replay of
       // transceiver status
...
nrf_err_t nrf_config_rx_channel(uint8_t channel, uint32_t addr,
                                uint8_t pack_size, uint8_t *buffer_point);
