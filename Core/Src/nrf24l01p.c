...
/* chip select*/
#define CSN(x)                                                                 \
  HAL_GPIO_WritePin(RF_MOSI_GPIO_Port, RF_MOSI_Pin,                                    \
                    (x == 0) ? GPIO_PIN_RESET : GPIO_PIN_SET)
/* chip enable */
#define CE(x)                                                                  \
  HAL_GPIO_WritePin(RF_CE_GPIO_Port, RF_CE_Pin,                                    \
                    (x == 0) ? GPIO_PIN_RESET : GPIO_PIN_SET)

#define delay(x) HAL_Delay(x)

#define write(data, size)                                                      \
  (int)HAL_SPI_Transmit(&hspi1, data, size, HAL_MAX_DELAY)
...
static nrf_err_t nrf_change_device_mode(nrf_mode_e mod) {
  nrf_err_t ret = NRF_OK;
  if (mod == nrf_mode) { // no need to change
    return NRF_OK;
  } else if (mod == NRF_MODE_Sleep_2) { // this mode can't be set directly
    return NRF_ERR_arg;
  } else if (mod ==
             NRF_MODE_Power_Off) { // the way every mode to this mode is same
    uint8_t CMD = 0x00;
    CE(0);
...
*/
nrf_err_t nrf_config_rx_channel(uint8_t channel, uint32_t addr,
                                uint8_t pack_size, uint8_t *buff_point) {
  if (channel > 5) {
    return -6;
  }
  if (buff_point != NULL) {
...
nrf_err_t nrf_set_enable(nrf_mode_e mod) {
  ...
  write_reg(nRF_SETUP_AW, &dat,
            1); // make address width = NRF_RFCHANNEL_ADDR_LENS_IN_BYTE
  dat = 0x3f;
...
nrf_mode_e nrf_get_mode(void) { return nrf_mode; }
